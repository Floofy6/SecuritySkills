...
| Windows Security 4688 | Windows | Process creation; command line only when Audit Process Creation and command-line process auditing are enabled | Process Creation (DC0032) |
| PowerShell Operational 4104 | Windows / PowerShell | Script block content when Script Block Logging is enabled | Command (DS0017), Process Creation (DC0032) |
...
#### Windows Command-Line and PowerShell Evidence Gates

Do not treat a process name alone as full execution evidence. MITRE ATT&CK Process Creation (DC0032) includes parent-child relationships, process arguments, and execution context; native Windows Security Event 4688 only includes the command-line field when the required audit policies are enabled.

Before assigning medium or higher severity to suspicious command execution, record the collection state:

| Evidence Gate | Required Check | Confidence Impact |
|---------------|----------------|-------------------|
| Audit Process Creation | Confirm Windows Security 4688 is generated by Advanced Audit Policy `Audit Process Creation` | If absent, native Windows process creation visibility is missing |
| Command-line process auditing | Confirm `Include command line in process creation events` is enabled and `Process Command Line` is present | If absent, downgrade process-name-only findings unless corroborated |
| Sensitive argument handling | Check whether command-line arguments include passwords, tokens, keys, connection strings, or user data | Redact sensitive values in reports; document collector access controls |
| PowerShell channel and version | Identify Windows PowerShell 5.1 (`Microsoft-Windows-PowerShell/Operational`) or PowerShell 7+ (`PowerShellCore/Operational`) | If unknown, document channel/version as a visibility gap |
| Script Block Logging | Confirm Event ID 4104 exists for the time window and contains relevant `ScriptBlockText` / script block content | If absent, treat PowerShell process evidence as incomplete |
| 4688 to 4104 correlation | Correlate user, host, process start time, parent process, command line, and script block content | If correlation fails, avoid claiming what script content executed |
| Sysmon / EDR fallback | Use Sysmon Event ID 1 or EDR process telemetry for full command line, parent, hashes, and process tree when native 4688 is incomplete | Document source precedence and collection health |
| Protected logging / redaction | For script block content, confirm Protected Event Logging or equivalent centralized protection/redaction when sensitive content may be captured | If unavailable, include only the minimum necessary redacted excerpts |

**Confidence rules:**

- Process name only (for example, `powershell.exe` or `cmd.exe`) is **low confidence** unless supported by command-line, script block, parent-child, network, file, authentication, or EDR evidence.
- `-EncodedCommand`, `Invoke-Expression`, download cradle patterns, or unusual parent processes can raise suspicion, but the report must state whether the encoded content was decoded and whether Script Block Logging or EDR confirmed execution.
- If Event ID 4688 exists without `Process Command Line`, file a visibility gap for command-line process auditing rather than inferring arguments from the image name.
- If PowerShell Event ID 4104 is missing, state whether Script Block Logging was disabled, outside retention, not centrally collected, or unavailable for the PowerShell version/channel in use.
...
### Evidence Confidence
| Evidence Type | Available? | Source / Channel | Notes |
|---------------|------------|------------------|-------|
| 4688 process creation | [Yes/No] | [Windows Security] | [Audit Process Creation state] |
| 4688 command line | [Yes/No] | [Process Command Line field] | [Command-line process auditing state; redactions applied] |
| PowerShell 4104 | [Yes/No] | [Microsoft-Windows-PowerShell/Operational or PowerShellCore/Operational] | [Script Block Logging state; script block correlation] |
| Sysmon / EDR process telemetry | [Yes/No] | [Sysmon EID 1 / EDR platform] | [Parent, command line, hashes, process tree] |
...
| Process | DS0009 | Process Creation (DC0032: parent-child relationships, process arguments, environmental variables), Process Access, Process Termination |
...
### Pitfall 6: Treating Process Names as Command Evidence

Seeing `powershell.exe`, `cmd.exe`, or `wscript.exe` in Event ID 4688 is not the same as proving the command or script content that ran. Event 4688 command-line fields require explicit command-line process auditing, and PowerShell script content requires Script Block Logging or equivalent EDR visibility. If arguments, decoded payloads, or Event ID 4104 script block content are missing, classify the result as a low-confidence observation or visibility gap unless corroborating evidence raises confidence.
...
5. **Microsoft Command Line Process Auditing** -- https://learn.microsoft.com/en-us/windows-server/identity/ad-ds/manage/component-updates/command-line-process-auditing
6. **PowerShell about_Logging (Windows PowerShell 5.1)** -- https://learn.microsoft.com/en-us/powershell/module/microsoft.powershell.core/about/about_logging?view=powershell-5.1
7. **PowerShell about_Logging_Windows (PowerShell 7+)** -- https://learn.microsoft.com/en-us/powershell/module/microsoft.powershell.core/about/about_logging_windows
8. **MITRE ATT&CK Process Creation (DC0032)** -- https://attack.mitre.org/datacomponents/DC0032/
